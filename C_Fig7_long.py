...
for isi, sigma in enumerate(sigmas):
    for ipop, S in enumerate(pops):
        ...
        for iti in range(trials):
            ...
            try:
                fl=np.load('Data/paramMF_sig_'+str(sigma)+ '_pop_'+str(S)+'_trial_'+str(iti)+'.npz')
                Sigma = fl['name1']
                sMx = fl['name2']
                sMy = fl['name3']
                sI = fl['name4']
                ...
                if os.path.isdir('Data/')==False:
                    os.mkdir('Data/')
                np.savez('Data/paramMF_sig_'+str(sigma)+ '_pop_'+str(S)+'_trial_'+str(iti),name1=Sigma, name2=sMx, name3=sMy, name4=sI, 
                         name5=muMx, name6=muMy, name7=MuN, name8=muI, name9=E, name10=sigmaU, name11=F0, name12=reconst)
                
            Ess[isi, iti, ipop] = E
            Mea[isi, iti, ipop] = np.mean(sigmaU)
            Var[isi, iti, ipop] = np.var(sigmaU)
            Max[isi, iti, ipop] = np.max(np.abs(sigmaU))
            ...
            if iti>-1:
                try:
                    fl = np.load('Data/dynamicsfinsiz_sig_'+str(sigma)+ '_pop_'+str(S)+'_trial_'+str(iti)+'.npz')
                    Mv = fl['name1']
                    Nv = fl['name2']
                    Xr = fl['name3']
                    Yr = fl['name4']
            ...
            try:
                fl = np.load('Data/comparison_sig_'+str(sigma)+ '_pop_'+str(S)+'_trial_'+str(iti)+'.npz')
                f0_fs=fl['name1']
            except:
                print('no comparison')
                f0_fs = np.zeros((p*K, 1))    
                for ip in range(p):
                    k = dat_point[ip, :]
                    k1 = dat_point[ip,0]
                    k2 = dat_point[ip,1]
                    SS = VP_funcnum(k1, k2, Mv, Nv, Iv)
                    f0_fs[ip*K]   = S*(SS[0] + k1)
                    f0_fs[ip*K+1] = S*(SS[1] + k2)
                np.savez('Data/comparison_sig_'+str(sigma)+ '_pop_'+str(S)+'_trial_'+str(iti), name1=f0_fs)
                
            E_fs = np.mean(np.abs((f0_fs.T-F0.T)))#/F0.T))
            Ess_fs[isi, iti, ipop] = E_fs
            if iti ==0:
                if verbose==True:
                    print('E_fs')
                    print(E_fs)
            E_r_fs = np.mean(np.abs((reconst - f0_fs.T)))
            Ess_r_fs[isi, iti, ipop] = E_r_fs
            if iti ==0:
                if verbose==True:
                    print('E_r_fs')
                    print(E_r_fs)
