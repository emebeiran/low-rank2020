...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 17 15:49:01 2020
...
ax_joint00.plot(ms, Sigma[0,0]*ms, '--', color ='k', lw=1)
...
ax_joint01.plot(ms, Sigma[0,1]*ms, '--', color ='k', lw=1)
...
ax_joint10.plot(ms, Sigma[1,0]*ms, '--', color ='k', lw=1)
...
ax_joint11.plot(ms, Sigma[1,1]*ms, '--', color ='k', lw=1)
...
for i in range(np.shape(Sigma)[0]):
    for j in range(np.shape(Sigma)[1]):
        ax.text(i, j, str(Sigma[i,j]), va='center', ha='center', fontsize=16)
...
ax.text(0.5, 0.2, r'$\lambda_1 \bf{u}_1$', fontsize = 18)
ax.text(0.1, 1.2, r'$\lambda_2 \bf{u}_2$', fontsize = 18)
...
plt.scatter( [0,0], [fp2, -fp2], s=100, edgecolor ='w', facecolor='k', linewidth=1.5, zorder=4)

ax.set_xticks([-1, 0, 1])
ax.set_yticks([-1, 0, 1])
ax.set_ylim([np.min(kaps2), np.max(kaps2)])
ax.set_xlim([np.min(kaps1), np.max(kaps1)])
plt.savefig('Th_Fig2_C1.pdf')
...
fig = plt.figure()
ax = fig.add_subplot(111) 
im = plt.pcolor(kaps1, kaps2, np.log10(E).T, cmap ='viridis', vmin = -2., shading='auto')
...
ax.set_ylim([np.min(kaps2), np.max(kaps2)])
ax.set_xlim([np.min(kaps1), np.max(kaps1)])
plt.savefig('Th_Fig2_C2.pdf')

#%%

fig = plt.figure()
ax = fig.add_subplot(111) 
#im = plt.pcolor(kaps1, kaps2, np.log10(E).T, cmap ='viridis', vmin = -2.)
...
SigmaTot[0,3] = Sigma[0,1]
SigmaTot[1,2] = Sigma[1,0]
...
inkap2 = np.linspace(-1.1, 1.1001, 4)
    ...
    for ik1, ink1 in enumerate(inkap1):
        for ik2, ink2 in enumerate(inkap2):
            ...
            for it, ti in enumerate(time[:-1]):
                ...
                x0 = x
            plt.plot(sk1, sk2, color =[cC, cC, cC])
            plt.scatter(sk1[0], sk2[0], s=10, facecolor=[cC, cC, cC])
